...
function renderPresetBar() {
  ...
  if (hasPresets) {
    ...
    menuHtml = '<div class="preset-empty">No saved profiles yet</div>';
  }
  menuHtml += '<div class="preset-sep"></div><div class="preset-item" onclick="_wiz.savePreset()" style="color:var(--accent);font-weight:600"><span class="preset-item-name">\uD83D\uDCBE Save Current Profile</span></div>';

  el.innerHTML = `<div class="preset-dd">
    <button class="preset-btn ${_presetMenuOpen ? 'open' : ''}" onclick="_wiz.togglePresetMenu()">
      <span class="preset-name">${hasPresets ? `\uD83D\uDCC2 ${names.length} profile${names.length > 1 ? 's' : ''}` : '\uD83D\uDCC2 Profiles'}</span>
...
/* ── Theme Swapper ── */
.wiz-scope .theme-swapper { display:flex;align-items:center;gap:4px;background:rgba(255,255,255,0.03);border:1px solid var(--brd);border-radius:12px;padding:4px; }
...
/* ── Rail Handle (mobile only — hidden on desktop) ── */
.wiz-scope .rail-handle { display:none; }
...
/* ── Presets ── */
.wiz-scope .preset-dd { position:relative;display:inline-flex;margin-left:auto; }
/* Preset button */
.wiz-scope .preset-btn { display:flex;align-items:center;gap:6px;padding:6px 14px;border-radius:10px;border:1px solid var(--brd);background:rgba(255,255,255,0.03);color:var(--text2);font-size:12px;font-weight:600;cursor:pointer;transition:all 0.2s;white-space:nowrap; }
.wiz-scope .preset-btn:hover { background:rgba(255,255,255,0.06);color:var(--text);border-color:rgba(255,255,255,0.12); }
.wiz-scope .preset-btn.open { border-color:var(--accent-glow);color:var(--text); }
.wiz-scope .preset-name { flex:1; }
.wiz-scope .preset-chev { font-size:10px;transition:transform .2s;color:var(--text3); }
.wiz-scope .preset-btn.open .preset-chev { transform:rotate(180deg); }
.wiz-scope .preset-dd .preset-menu { position:absolute;top:calc(100% + 6px);right:0;min-width:240px;background:var(--bg-card);border:1px solid var(--brd);border-radius:14px;box-shadow:0 8px 32px rgba(0,0,0,.35);z-index:10;padding:8px 0;display:none; }
...
.wiz-scope .preset-item-date { font-size:11px;color:var(--text3);white-space:nowrap; }
.wiz-scope .preset-del { background:none;border:none;color:var(--text3);font-size:16px;cursor:pointer;padding:0 2px;opacity:0;transition:opacity .15s,color .15s;line-height:1; }
.wiz-scope .preset-item:hover .preset-del { opacity:1; }
.wiz-scope .preset-del:hover { color:#ff6b6b; }
.wiz-scope .preset-empty { padding:12px 16px;font-size:12px;color:var(--text3);font-style:italic;text-align:center; }
