# StratOS — New Chat Context Handoff

## What Is StratOS

StratOS is a personalized strategic intelligence dashboard. It fetches news from multiple sources (RSS, DuckDuckGo, Serper/Google), scores them for relevance using a local Ollama-hosted LLM, and presents a prioritized feed via a web UI. It has a self-improving scoring model via Claude Opus distillation → LoRA fine-tuning. Single-user system, SQLite backend, Python HTTP server, vanilla JS frontend.

**Key tech:** Python 3.12, vanilla JS (no framework), SQLite WAL, Ollama (Qwen3), SSE for live updates, YAML profiles per user.

---

## What Happened Today (8 sessions)

### Sessions 1–4: Security Audit
A comprehensive security audit was performed and committed in 5 phases:
...
**181 insertions, 96 deletions, 17 files.** These are already committed to `main`.

### Sessions 5–6: UI/UX Bug Identification & Implementation Attempt
I identified 10 UI/UX bugs/features. Implementation was done as standalone file patches (delivered to outputs/). These were **superseded** by the Claude Code prompt approach.

### Sessions 7–8: Claude Code Prompt Creation
Instead of manual patches, I created a comprehensive Claude Code prompt (`CLAUDE_CODE_PROMPT.md`) with exact line references, git workflow, audit coordination, and verification passes. **This is the current deliverable — nothing has been applied to the codebase yet.**

---

## Current State

- **Security audit:** ✅ Committed to main (5 commits)
- **10 UI/UX fixes:** ❌ NOT YET APPLIED — ready as a Claude Code prompt
- **Branch:** The work should go on `feature/ui-fixes-and-polish` branched from current `main`

---

## The 10 Changes (Summary)

1. **Username Login** — `server.py`: Allow login with display_name (resolves to email via DB lookup)
2. **Tour Persistence** — `auth.js`: Preserve `stratos_tour_never` in `_clearProfileLocalStorage()`
3. **Animate-Spin CSS** — `styles.css`: Add missing `.animate-spin` class (used 20+ places, keyframes exist but class doesn't)
4. **Progress Bar Polling** — `app.js`: Add `'news_done'` stage to polling fallback (SSE handler has it, poll doesn't)
5. **Theme Editor Sliders** — `theme-editor.js` + `styles.css`: Add "Effects" group with 5 range inputs (card opacity, panel opacity, border radius, glow intensity, glass blur)
6. **Per-Card Refresh** — `markets-panel.js`: Add refresh button to each market chart card header
7. **Agent Warmup** — `app.js` + `server.py`: Pre-load Ollama model on page load via backend proxy (CORS fix — can't hit Ollama directly from browser)
8. **Email Auto-Populate** — `server.py`: DB fallback for email in `/api/status` for DB-auth users (YAML users already get it)
9. **Context Box Move** — `index.html`: Split "Quick Profile" → Profile tab (role/location) + Sources tab (context/buttons). Add strat-tip tooltips. Add agent tool badges.
10. **Wizard Glassmorphism** — `wizard.js`: CSS-only polish to `WIZ_CSS` string (blur, noise texture, deeper shadows, smoother transitions)

---

## Critical Coordination Notes

- **server.py** is the most-touched file: audit moved rate limiting before route dispatch, masked errors. New changes (fixes 1, 7, 8) must slot in cleanly.
- **index.html** has SRI integrity hashes from Phase 5 — DOM restructuring (fix 9) must NOT touch `<script>` tags.
- **auth.js** has XSS escaping from Phase 1 — tour fix (fix 2) is in a separate function, should be clean.
- **wizard.js** has rate limits from Phase 3 — CSS changes (fix 10) don't touch logic.

---

## The Full Implementation Prompt

The complete Claude Code prompt is below. It includes exact line numbers (which may have shifted slightly from the audit commits — Claude Code should verify), code snippets, edge cases, 2 refinement passes, and final verification steps.
...
- `index.html`: Phase 5 added SRI integrity hashes on CDN `<script>` tags. Your DOM restructuring (Fix 9) must not remove or alter those.
...
```
backend/
  main.py          — StratOS class, scan pipeline, entry point
  server.py        — HTTP server, all API routes (CORSHandler)
  auth.py          — AuthManager, sessions, rate limiting
...
frontend/
  ...
  app.js           — Main frontend logic, init(), scan handling
  auth.js          — Client-side auth, localStorage management
  settings.js      — Settings panel tabs and save logic
  feed.js          — News feed rendering
  markets-panel.js — Market chart cards
  theme-editor.js  — Theme customization panel
  wizard.js        — Onboarding wizard (embedded CSS + HTML)
  tour.js          — Interactive tour
  agent.js         — Chat agent UI
  nav.js           — Navigation
