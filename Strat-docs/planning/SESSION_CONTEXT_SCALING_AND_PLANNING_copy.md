...
14. [OCR & Vision Models — Research Findings](#14-ocr--vision-models)
...
export OLLAMA_MAX_LOADED_MODELS=2    # or 3 with PaddleOCR-VL (see §14)
...
## 14. OCR & Vision Models — Research Findings (March 2026)

### Context

StratOS needs image/document upload capability: screenshot OCR, TCG card identification, PDF extraction, paywalled article reading. The approach is lightweight OCR model → extracted text → existing Strat Agent (qwen3.5:9b) for reasoning. The OCR model only extracts text; the LLM interprets it. Same pattern as Whisper (audio → text → agent).

### The OCR Landscape Shifted (Oct 2025 — Mar 2026)

Traditional OCR engines (Tesseract, old PaddleOCR, EasyOCR) are now outdated. A wave of Vision Language Models (VLMs) fine-tuned specifically for OCR dropped in late 2025, achieving accuracy that matches or exceeds commercial APIs (Azure, Google Vision, AWS Textract) while running locally.

### Current State of the Art (March 2026)

| Model | Params | VRAM | On Ollama? | Best at | Released |
|-------|--------|------|-----------|---------|----------|
| **PaddleOCR-VL 1.5** | 0.9B | ~1GB | ✅ Yes | Documents, tables, formulas, 111 languages, warped/skewed photos. 94.5% on OmniDocBench v1.5 — SOTA for size class. Works on AMD ROCm. | Jan 2026 |
| **OCRFlux-3B** | 3B | ~3GB | No (vLLM) | Single-page PDF→Markdown. 0.967 edit distance — beats olmOCR-7B | Late 2025 |
| **Dots.OCR-3B** | 3B | ~3GB | No | Highest character-level accuracy in Feb 2026 benchmarks | Mid 2025 |
| **olmOCR-2-7B** | 7B | ~5GB | No (vLLM) | Handwriting (second only to GPT-5), scanned PDFs, equations | Late 2025 |
| **DeepSeek-OCR-3B** | 3B | ~3GB | No | Efficient MoE, good accuracy per VRAM | Oct 2025 |
| **MiniCPM-o 2.6** | 8B | ~5GB | ✅ Yes | Tops OCRBench leaderboard — beats GPT-4o and Gemini 1.5 Pro. 30+ languages | 2025 |
| **RolmOCR** | 7B | ~5GB | No | Fine-tune of Qwen2.5-VL specifically for OCR | Late 2025 |
| **SmolDocling** | 256M | ~0.5GB | No | Processes documents in 0.35s on consumer GPUs. Tiniest model. | Mar 2026 |
| **HunyuanOCR** | ~1B | ~1.5GB | No | Strong on CJK (Chinese/Japanese/Korean) text | Oct 2025 |

### Best Model Per Use Case

| Use Case | Best Model | Why | VRAM |
|----------|-----------|-----|------|
| Screenshots, articles, paywalled content | PaddleOCR-VL 1.5 | SOTA at 0.9B, on Ollama, handles any doc | ~1GB |
| TCG card photos (name, set, rarity) | PaddleOCR-VL 1.5 | Handles angled, warped, varied lighting | ~1GB |
| PDFs / academic papers | OCRFlux-3B or olmOCR-2-7B | Best at multi-column, equations, complex layout | 3-5GB |
| Handwriting | olmOCR-2-7B | Second only to GPT-5 on benchmarks | ~5GB |
| Speed-critical (sub-second) | SmolDocling (256M) | 0.35s per doc, barely uses resources | ~0.5GB |
| Best overall (unlimited VRAM) | MiniCPM-o 2.6 | Tops OCRBench, beats GPT-4o | ~5GB |

### Decision: PaddleOCR-VL 1.5 (0.9B)

**This is the model to use.** Rationale:
- SOTA accuracy at 0.9B parameters — matches models 3-8x its size
- Only ~1GB VRAM — fits alongside scorer + inference with 8GB headroom
- Already on Ollama: `ollama pull MedAIBase/PaddleOCR-VL:0.9b`
- Confirmed working on AMD ROCm (Ahmad's hardware)
- Handles every use case: screenshots, card photos, PDFs, Arabic text, tables, formulas
- 111 languages including Arabic (critical for Kuwait localization)

### VRAM Budget with OCR Model

```
stratos-scorer-v2.2 (Q8_0):     ~8.7GB
qwen3.5:9b (Q4_K_M):            ~5.5GB
PaddleOCR-VL 1.5 (0.9B):        ~1.0GB
KV cache (all models):           ~1-2GB
Total:                           ~16-17GB
Headroom:                        ~7-8GB ✓
...
Image/PDF upload
    ↓
PaddleOCR-VL 1.5 (Ollama, ~1GB VRAM, <1s)
    ↓
Structured text / markdown output
    ↓
Strat Agent (qwen3.5:9b) — interprets, summarizes, answers questions
    "Summarize this article"
    "What card is this? Get me the price."
    "Extract the key findings from this PDF."

Audio upload
    ↓
Whisper.cpp (CPU, 0 VRAM, ~2-5s)
    ↓
Transcribed text
    ↓
Strat Agent (same pipeline)
```

The agent never "sees" images or hears audio. It receives text. Zero additional VRAM beyond the OCR model itself. The heavy lifting is always the text LLM interpreting the extracted content.

### Per-Mode OCR Use Cases

| Mode | Use Case | Flow |
|------|----------|------|
| **Intelligence** | Upload paywalled article screenshot → OCR → summarize | Image → PaddleOCR-VL → text → agent summary |
| **Intelligence** | Upload PDF report → OCR → extract key findings | PDF → PaddleOCR-VL → markdown → agent analysis |
| **Anime/Manga** | Upload screenshot → identify show/character (limited — OCR reads on-screen text, can't identify characters visually) | Image → PaddleOCR-VL → text (subtitles, titles) → agent lookup |
| **Collection/TCG** | **Killer feature.** Photo of card → extract name + set number → API price lookup → add to collection | Photo → PaddleOCR-VL → "Blue-Eyes White Dragon LOB-001" → YGOPRODeck API → price + metadata |
| **Collection/TCG** | Photo of card binder page → batch extract multiple cards | Photo → PaddleOCR-VL → multiple card names → batch API lookup |
| **Gaming** | Screenshot of error/settings → troubleshoot | Image → PaddleOCR-VL → text → agent diagnosis |

### Visual Understanding Gap

PaddleOCR-VL extracts text from images but does NOT understand visual content. It cannot:
- Identify anime characters from a screenshot (no character recognition)
- Assess card condition from a photo (no visual quality judgment)
- Recognize game scenes or UI elements by appearance

For these tasks, you'd need a general vision-language model like Qwen2.5-VL-3B (~3.5GB) or MiniCPM-o 2.6 (~5GB). These are optional upgrades — the OCR-first approach covers 90% of use cases. The remaining 10% (visual identification) can be added later if demand warrants the VRAM cost.

### Deployment Commands

```bash
# Install PaddleOCR-VL 1.5 via Ollama
ollama pull MedAIBase/PaddleOCR-VL:0.9b

# Test it
curl http://localhost:11434/api/chat -d '{
  "model": "MedAIBase/PaddleOCR-VL:0.9b",
  "messages": [{"role": "user", "content": "OCR:", "images": ["BASE64_IMAGE_HERE"]}]
}'

# Update Ollama env to keep all 3 models loaded
export OLLAMA_MAX_LOADED_MODELS=3
export OLLAMA_KEEP_ALIVE=-1
```

### Models NOT Recommended (Outdated)

These were considered earlier in the session but are now superseded:
- **Tesseract** — PaddleOCR-VL is better in every dimension
- **EasyOCR** — slower, less accurate than PaddleOCR-VL
- **Old PaddleOCR (pip package)** — different from PaddleOCR-VL; the old one is a traditional pipeline, the new VL is a vision-language model
- **Surya** — good for documents but PaddleOCR-VL 1.5 matches it at 1/3 the size
- **Qwen2.5-VL-3B** — general vision model, not OCR-specialized. PaddleOCR-VL is better at text extraction specifically
