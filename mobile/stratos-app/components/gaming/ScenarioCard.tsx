...
export function ScenarioCard({ scenario, variant = 'full' }: ScenarioCardProps) {
  ...
  if (variant === 'horizontal') {
    return (
      <TouchableOpacity style={[styles.horizontalCard, { borderColor: accentColor + '30' }]} onPress={() => router.push(`/gaming/${scenario.id}`)} activeOpacity={0.7}>
        ...
        <Text style={styles.horizontalName} numberOfLines={1}>{scenario.name}</Text>
        <Text style={[styles.horizontalGenre, { color: accentColor }]}>{scenario.subgenre}</Text>
      </TouchableOpacity>
    );
  }
  return (
    <TouchableOpacity style={styles.card} onPress={() => router.push(`/gaming/${scenario.id}`)} activeOpacity={0.7}>
      <View style={[styles.cardHeader, { borderLeftColor: accentColor }]}>
        ...
        <View style={styles.cardMeta}>
          ...
          <View style={styles.ratingRow}><Star size={12} color={colors.accent.secondary} fill={colors.accent.secondary} /><Text style={styles.ratingText}>{scenario.rating.toFixed(1)}</Text></View>
        </View>
      </View>
      <Text style={styles.description} numberOfLines={2}>{scenario.description}</Text>
      <Text style={styles.sessions}>{formatCount(scenario.session_count)} sessions</Text>
    </TouchableOpacity>
  );
}

const styles = StyleSheet.create({
  card: { backgroundColor: colors.bg.secondary, borderRadius: borderRadius.lg, padding: spacing.lg, borderWidth: 1, borderColor: colors.border.subtle, gap: spacing.sm },
  cardHeader: { borderLeftWidth: 3, paddingLeft: spacing.md },
  cardName: { ...typography.heading, color: colors.text.primary, marginBottom: spacing.xs },
  ...
  horizontalCard: { width: 140, alignItems: 'center', marginRight: spacing.md, backgroundColor: colors.bg.secondary, borderRadius: borderRadius.lg, padding: spacing.md, borderWidth: 1 },
  ...
  horizontalName: { ...typography.caption, color: colors.text.primary, fontWeight: '600', textAlign: 'center', marginBottom: 2 },
